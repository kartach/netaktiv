...
class N2GeneratorEasySocialEvents extends N2GeneratorAbstract
{
    ...
    protected function _getData($count, $startIndex) {
        ...
        $query = "SELECT
                  ...
                  (SELECT value FROM #__social_photos_meta WHERE photo_id = (SELECT photo_id FROM #__social_covers WHERE uid = a.id and type='event' LIMIT 1) AND property = 'original') AS original_cover,
                  (SELECT value FROM #__social_photos_meta WHERE photo_id = (SELECT photo_id FROM #__social_covers WHERE uid = a.id and type='event' LIMIT 1) AND property = 'thumbnail') AS thumbnail_cover,
                  (SELECT value FROM #__social_photos_meta WHERE photo_id = (SELECT photo_id FROM #__social_covers WHERE uid = a.id and type='event' LIMIT 1) AND property = 'square') AS square_cover,
                  (SELECT value FROM #__social_photos_meta WHERE photo_id = (SELECT photo_id FROM #__social_covers WHERE uid = a.id and type='event' LIMIT 1) AND property = 'featured') AS featured_cover,
                  (SELECT value FROM #__social_photos_meta WHERE photo_id = (SELECT photo_id FROM #__social_covers WHERE uid = a.id and type='event' LIMIT 1) AND property = 'large') AS large_cover,
                  (SELECT value FROM #__social_photos_meta WHERE photo_id = (SELECT photo_id FROM #__social_covers WHERE uid = a.id and type='event' LIMIT 1) AND property = 'stock') AS stock_cover
