...
(function($) {
    ...
    $.datePicker = {
        default: {
            ...
            dateFormatType: null,                    // date format types
    ...
    $.fn.datePicker = function (option) {
        ...
        var newOption = $.extend(
            ...
            $.datePicker.default,
            {
                ...
                isHoliday: function (moment) {
                    var dateFormattypes = this.getDateFormat();
                    if($.isFunction(this.holiday) && this.holiday(moment))
                        return true;

                    //noinspection RedundantIfStatementJS
                    if($.isArray(this.holiday) && 0 <= $.inArray(moment.format(dateFormattypes), newOption.holiday))
                        return true;
                ...
                getDateFormat: function(){
                    ...
                    switch(this.dateFormatType){
                        ...
                        case "m/d/Y":
                            return 'L';
                        break;
                        //DD-MM-YYYY format is not supported by the jquery.datePicker.js
                        case "m-d-y":
                            return 'll';
                        break;
                        /*case "h d Y":
       ...
       // if($.type(newOption.dateFormatType) == 'string')
         //   newOption.dateFormatType = [newOption.dateFormatType];

        (function ($input, newOption) {

            $($input).focusin(function () {
                var dateformatTypes = newOption.getDateFormat();
                var $body = $('body');
                var offset = $input.offset();
                var val = moment($input.val()).format(dateformatTypes);
                if(!moment($input.val()).isValid())
                    val = moment().format(dateformatTypes);
...
//                var momentForPrint = moment(display).add(-Math.floor((newOption.monthCount - 1) / 2), 'M');
                ...
                for(var i = 0; i < newOption.monthCount; i++){
                    tr.append(
                        tag('td')
                            ...
                            .append(drawMonth($input, newOption, selected, momentForPrint.format(dateformatTypes)))
                    );
            ...
            function drawMonth($input, newOption, selected, display) {
                ...
                var $table = tag('table').addClass(newOption.cssClass.month).attr('data-date', momentDisplay.format(dateformatTypes)).css({ borderCollapse: 'collapse' }).append(
                    tag('tr')
                        .append(tag('td').addClass(newOption.cssClass.prev))
                        .append(tag('td').addClass(newOption.cssClass.yearMonth).attr('colspan', '5').css({ textAlign: 'center' }).text(momentDisplay.format('YYYY-MMM')))
                ...
                for(var i = 1; i <= lastDay; i++) {
                    ...
                    var $td = tag('td')
                        .attr('data-date', momentForPrint.format(dateformatTypes))
                        .text(momentForPrint.date())
                        .addClass(newOption.cssClass.date)
                        .addClass(moment(new Date()).format(dateformatTypes) == momentForPrint.format(dateformatTypes) ? newOption.cssClass.today : '')
                        .addClass(momentSelected.format(dateformatTypes) == momentForPrint.format(dateformatTypes) ? newOption.cssClass.selected : '')
