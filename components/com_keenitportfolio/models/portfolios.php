...
if (!JFactory::getUser()->authorise('core.edit.state', 'com_keenitportfolio'))
{
	$query->where('a.state = 1');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.project_name LIKE '.$search.'  OR  a.client_name LIKE '.$search.'  OR  a.project_url LIKE '.$search.'  OR  a.desc LIKE '.$search.' )');
			}
		}
		...
		foreach($items as $item){
			...
			if ( isset($item->category) ) {
				...
				// Get the title of that particular template
					$title = KeenitportfolioFrontendHelper::getCategoryNameByCategoryId($item->category);
