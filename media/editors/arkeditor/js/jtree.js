...
Script: jtree.js
	...
	- Developed by Webxsolution ltd
	- Base on Mootree developed by Rasmus Schultz, <http://www.mindplay.dk>
...
jQuery(function($)
{
	...
	var jTreeIcon = ['I','L','Lminus','Lplus','Rminus','Rplus','T','Tminus','Tplus','_closed','_doc','_open','minus','plus'];


	var jTreeControl =  function(config, options) {
			...
			this.root = new jTreeNode(options); // create the root node of this tree control
					...
					for (n=0; n<s.length; n++) {
						switch (s[n].nodeName) {
							case 'A':
								...
								con.data = {};
								$.each(['href','target','title','name'], function( index, value) {
										con.data[value] = $(s[n]).attr(value) || null;
								});
							break;
							case 'UL':
								subul = s[n];
	...
	*/
		...
		this.div = {
			main:	$('<div></div').addClass('mooTree_node').get(0),
			indent:	$('<div></div').get(0),
			gadget:	$('<div></div').get(0),
			icon:	$('<div></div').get(0),
			text:	$('<div></div').addClass('mooTree_text').get(0),
			sub:	$('<div></div').get(0)
		}
		...
		getElement(options.div).append(this.div.main);
		getElement(options.div).append(this.div.sub);
	...
	$.extend(jTreeNode.prototype,	
	{
		insert: function(options) {
			...
			var node = new jTreeNode(options);
		...
		*/
		...
		Parameters:
			name - the name of new image to create, defined by <jTreeIcon> or located in an external file.
		...
		*/
		...
		load: function(url, vars) {
			...
			var f = function() {
				...
				).done($.proxy(function(xml) {
						this._loaded(xml);					
					},this)
				).fail($.proxy(function(resp) {
						this.f(resp);
					},this)
				)
			}
			setTimeout($.proxy(f,this),20); // allowing a small delay for the browser to draw the loader-icon.
		},

		_loaded: function(xml) {
	...
	$.fn.jtree = function( config, options) {
		config.div = this;
		return new jTreeControl( config, options);
	}
