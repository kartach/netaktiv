...
class modIceMegamenuHelper
{
	...
	function &getXML($type, &$params, $decorator)
	{
		...
		// Get document
		require_once(JPATH_BASE."/modules/mod_icemegamenu/libs/simplexml.php");
		$xml = new JSimpleXML;
	...
	{				
		switch($params->get('menu_style', 'list'))
		{
			...
			default :
				...
				$xml = self::getXML($params->get('menutype'), $params, $callback);
				if($xml)
				{
					...
					$result = JFilterOutput::ampReplace($xml->toString(true));
	...
	public function loadMediaFiles($params, $module)
    {
        ...
        if (!defined("MOD_ICEMEGAMENU")) {
            ...
            $css_path = JPATH_ROOT . DS . "templates" . DS . $app->getTemplate() . DS . "css" . DS;
            if ($resizable_menu == 1) {
                //$css3 = $css_path . "icemegamenu-reponsive.css";
            }
            if (file_exists($css_path . "template.css")) {
               /* $document->addStyleSheet(JUri::root(true) . "/templates/" . $app->getTemplate() . "/css/" . "template.css");*/
            } else {
                $css = 'modules/' . $module->module . '/themes/' . $params->get('theme_style', 'default') . '/css/' . $theme_style . '_icemegamenu.css';
                $document->addStyleSheet($css);
            }
            /*if (file_exists($css3)) {
                $document->addStyleSheet(JUri::root(true) . "/templates/" . $app->getTemplate() . "/css/" . "icemegamenu-reponsive.css");
            } elseif ($resizable_menu == 1) {
                $css3 = 'modules/' . $module->module . '/themes/' . $params->get('theme_style', 'default') . '/css/' . $theme_style . '_icemegamenu-reponsive.css';
