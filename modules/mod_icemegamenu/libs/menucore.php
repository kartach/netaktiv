...
class IceMenuTree extends JTree
{
	...
	function toXML($vertical_direction)
	{
		...
		if($this->_buffer == '') { $this->_buffer = "<ul></ul>\r\n"; }
		return $this->_buffer;
	}
	...
	function _getLevelXML($depth, $widthParent=0, $cols_widthParent=0, $colsParent=0, $arr_cols=array(), $parentId=0, $countChild=0, $colsWidth = 0)
	{
	   ...
	   if($this->_current->hasChildren()) {$ClassStyleLi = $ClassStyleLi." mzr-drop parent";}
       	$active_class = $this->_current->getActiveClass();
	...
	function _getItemData(&$params, $item)
	{
		...
		switch($tmp->type)
		{
			case 'separator' :
				...
				if($iParams->get("icemega_subtype") == 'anch') {
		            ...
		            $parse = JURI::getInstance();
		            $query = $router->parse($parse); // Get the real joomla query as an array - parse current joomla link
		            $url = 'index.php?'.JURI::getInstance()->buildQuery($query);
					$tmp->url = $iParams-
					$tmp->url = $iParams->get('icemega_anchorsmenuitem') == $this->_currentItemId && strpos($url, $menu->link) === 0 ? '#'.$tmp->alias: 'index.php?Itemid='.$iParams->get('icemega_anchorsmenuitem').'#'.$tmp->alias;
			...
			default :
				...
				if($iParams->get("icemega_subtype") == 'anch') {
		            ...
		            $query = $router->parse($parse); // Get the real joomla query as an array - parse current joomla link
		            $url = 'index.php?'.JURI::getInstance()->buildQuery($query);
					if($this->_currentItemId == $iParams->get('icemega_anchorsmenuitem') && strpos($url, $menu->link) === 0){
						$tmp->url = '#'.$tmp->alias;
						$tmp->anchor = ' data-anchor="'.$iParams->get('icemega_anchors').'"';
					}
				}
		...
		if($tmp->url != null && $iParams->get("icemega_showlink",1))
		{
			...
			switch($tmp->browserNav)
			{
				...
				case 0:
					// _top
					$data = '<a href="'.$tmp->url.'" title="'.htmlspecialchars($item->params->get('menu-anchor_title', ''), ENT_COMPAT, 'UTF-8', false).'" class="'.$active_class.' '.$myClass.'">'.$image.$tmp->name.'</a>';
					break;
				case 1:
					// _blank
					$data = '<a href="'.$tmp->url.'" target="_blank" title="'.htmlspecialchars($item->params->get('menu-anchor_title', ''), ENT_COMPAT, 'UTF-8', false).'"  class="'.$active_class.' '.$myClass.'">'.$image.$tmp->name.'</a>';
					break;
				case 2:
					// window.open
					...
					$data = '<a href="'.$link.'" onclick="window.open(this.href,\'targetWindow\',\''.$attribs.'\');return false;" title="'.htmlspecialchars($item->params->get('menu-anchor_title', ''), ENT_COMPAT, 'UTF-8', false).'" class="'.$active_class.' '.$myClass.'">'.$image.$tmp->name.'</a>';
					break;
			}
		} else {
...
class IceMenuNode extends JNode
{
    ...
    public function getActiveClass(){
		...
		if($this->active){
			...
			$path		= $this->active->tree;
		}
		$aliasToId = $this->params->get('aliasoptions');
       	if (isset($active_id) && ($this->id == $active_id || $aliasToId == $active_id)){
            $active_class .= " current";
       	}
        if (isset($path) && (in_array($this->id, $path) || in_array($aliasToId, $path)))
		{
			$active_class .= ' active';
			return $active_class;
		}
		elseif ($this->hasChildren()){
			foreach($this->getChildren() as $child){
	       		$active_class .= $child->getActiveClass();
	       	}
	       	return $active_class;
       	}
   	}
