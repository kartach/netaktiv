 ...
 */
class JHotelReservationModelTaxes extends JModelList
{
    ...
    /**
     * Method to get All hotel data list
     * @return mixed
     */
	function getHotels()
	{
    ...
    protected function getListQuery()
    {
        ...
        $query->select($this->getState('list.select', 'ht.*'));
        $query->from($db->quoteName('#__hotelreservation_taxes').' AS ht');

        //Join the hotels table with the country table
        $query->select('hh.hotel_id');
        $query->join('LEFT' , $db->quoteName('#__hotelreservation_hotels').' AS hh ON hh.hotel_id = ht.hotel_id ');

        $hotelId = $this->getState('filter.hotel_id');
        if (is_numeric($hotelId)) {
            $query->where('ht.hotel_id ='.(int) $hotelId);
        }
        $query->group('ht.tax_id');
    ...
    protected function populateState($ordering = null , $direction = null) {
        ...
        $statusId = $app->getUserStateFromRequest($this->context.'.filter.tax_id', 'filter_tax_id');
        $this->setState('filter.tax_id', $statusId);

		$hotelId = JRequest::getVar("hotel_id");
        if(empty($hotelId ))
        	$hotelId = $app->getUserStateFromRequest($this->context.'.filter.hotel_id', 'filter_hotel_id');
        ...
        parent::populateState('ha.tax_id', 'desc');
    }
