 ...
 * @subpackage  com_jhotelreservation
 */
class JHotelReservationModelRatingClassifications extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				'id', 'rc.id',
				'title', 'rc.name',
				'min_rate', 'rc.min_rate',
				'max_rate', 'rc.max_rate',
				'oredering', 'rc.ordering'
			);
		}
	...
	protected function getListQuery()
	{
		...
		$query->select($this->getState('list.select', 'rc.*'));
		
		$query->from($db->quoteName('#__hotelreservation_rating_classification').' AS rc');
		
		$query->group('rc.id');

		// Add the list ordering clause.
		$query->order($db->escape($this->getState('list.ordering', 'rc.id')).' '.$db->escape($this->getState('list.direction', 'ASC')));
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		parent::populateState('rc.id', 'desc');
	}
	...
	public function reorder($pk, $direction = 0)
	{
		...
		$pk	= (!empty($pk)) ? $pk : (int) $this->getState('ratingclassification.id');
		$user = JFactory::getUser();

		// Get an instance of the record's table.
		$table = JTable::getInstance('RatingClassifications','JTable');
